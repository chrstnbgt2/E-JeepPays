...
const DisplayAllQR_Conductor = () => {
  ...
  const getLocation = async () => {
    try {
      ...
      Alert.alert('Warning', 'Failed to get current location.');
      throw error;
    }
  };
  ...
  const handleStartTrip = async item => {
    try {
      ...
      const currentUser = auth().currentUser;
      if (!currentUser) {
        Alert.alert('Warning', 'Conductor is not logged in.');
        return;
      }
      ...
      const tripRef = database().ref(
        `/trips/temporary/${conductorUid}/${item.key}`,
      );
      const tripSnapshot = await tripRef.once('value');
      if (
        tripSnapshot.exists() &&
        tripSnapshot.val().status === 'in-progress'
      ) {
        Alert.alert('Warning', 'This trip is already in progress.');
        return;
      }
      ...
      if (!jeepneySnapshot.exists()) {
        Alert.alert('Warning', 'Jeepney not found.');
        return;
      }

      let jeepneyData = jeepneySnapshot.val();
      if (jeepneyData.currentCapacity <= 0) {
        Alert.alert('Warning', 'The jeepney is full.');
        return;
      }
      ...
      console.error('Warning starting trip:', error);
      Alert.alert('Warning', 'Failed to start the trip.');
    } finally {
  ...
  const handleStopTrip = async item => {
    try {
      ...
      if (!tripSnapshot.exists()) {
        Alert.alert('Warning', `No active trip found for ${item.username}.`);
        return;
      }
      ...
      if (!fareSnapshot.exists()) {
        Alert.alert('Warning', `Fare type "${fareType}" not found.`);
        return;
      }
      ...
      if (!response.ok || !routeData.routes || routeData.routes.length === 0) {
        Alert.alert('Warning', 'Failed to calculate route.');
        return;
      }
      ...
      const dailyStatsRef = database().ref(
        `/jeepneys/${jeepneyId}/dailyStats/${today}`,
      );
      ...
      await dailyStatsRef.update({
        ...
        cashPayment: parseFloat(
          (dailyStatsCash.cashPayment + payment).toFixed(2),
        ),
      });

      // Log transaction for the conductor
      const conductorTransactionsRef = database().ref(
      ...
      Alert.alert(
        'Trip Stopped',
        `Trip for ${item.username} completed. Distance: ${(
          distanceMeters / 1000
        ).toFixed(2)} km. Payment: ₱${payment.toFixed(
          2,
        )}. Seats available: ${updatedCapacity}`,
      );
    } catch (error) {
      console.error('Error stopping trip:', error);
      Alert.alert('Warning', 'Failed to stop the trip.');
    } finally {
